...
class GameScene extends Phaser.Scene {
    ...
    updateOpponentAI() {
        if (!this.opponent || !this.mainSTX) return;
    ...
    checkCollisions() {
        ...
        if (this.mainSTX && CollisionSystem.checkPointCollision(
            ...
            this.collectMainSTX('player');
        }
        
        // Check opponent main STX collision
        if (this.opponent && this.mainSTX && CollisionSystem.checkPointCollision(
            this.opponent.x, this.opponent.y,
            this.mainSTX.x, this.mainSTX.y, 25
        )) {
            this.collectMainSTX('opponent');
        }
    ...
    collectMainSTX(who = 'player') {
        ...
        if (who === 'player') {
            ...
            this.opponentSTXCount = (this.opponentSTXCount || 0) + 1;
        }
    ...
    showGameOverScreen(winner, playerSTX, opponentSTX) {
        ...
        if (finalScoreElement) {
            const playerIsWinner = playerSTX > opponentSTX;
            const villainIsWinner = opponentSTX > playerSTX;
            const isDraw = playerSTX === opponentSTX;
            ...
            finalScoreElement.innerHTML = `
                <div style="display: flex; gap: 20px; justify-content: center; align-items: stretch; width: 100%; max-width: 600px; margin: 20px auto;">
                    ...
                    <div style="
                        flex: 1;
                        background: ${playerIsWinner ? 'linear-gradient(135deg, #10b981, #059669)' : 'linear-gradient(135deg, #374151, #1f2937)'};
                        border: ${playerIsWinner ? '3px solid #10b981' : '2px solid #4b5563'};
                        border-radius: 15px;
                        padding: 25px 20px;
                        text-align: center;
                        box-shadow: ${playerIsWinner ? '0 8px 30px rgba(16, 185, 129, 0.4)' : '0 4px 15px rgba(0, 0, 0, 0.3)'};
                        transform: ${playerIsWinner ? 'scale(1.05)' : 'scale(1)'};
                        transition: all 0.3s ease;
                    ">
                        ${playerIsWinner ? '<h1 style="font-size: 32px; color: #ffffff; margin: 0 0 15px 0; text-shadow: 0 0 20px rgba(16, 185, 129, 0.8);">🏆 WINNER 🏆</h1>' : ''}
                        <h2 style="font-size: 24px; color: #00ccff; margin: 0 0 15px 0; text-shadow: 0 0 15px #00ccff;">🐭 Player</h2>
                        <div style="font-size: 48px; color: #ffffff; font-weight: bold; margin: 15px 0; text-shadow: 0 0 20px rgba(255, 255, 255, 0.5);">${playerSTX}</div>
                        <div style="font-size: 16px; color: #d1d5db; margin-top: 10px;">STX Tokens Collected</div>
                    </div>
                    
                    <!-- Villain Side -->
                    <div style="
                        flex: 1;
                        background: ${villainIsWinner ? 'linear-gradient(135deg, #7c3aed, #5b21b6)' : 'linear-gradient(135deg, #374151, #1f2937)'};
                        border: ${villainIsWinner ? '3px solid #7c3aed' : '2px solid #4b5563'};
                        border-radius: 15px;
                        padding: 25px 20px;
                        text-align: center;
                        box-shadow: ${villainIsWinner ? '0 8px 30px rgba(124, 58, 237, 0.4)' : '0 4px 15px rgba(0, 0, 0, 0.3)'};
                        transform: ${villainIsWinner ? 'scale(1.05)' : 'scale(1)'};
                        transition: all 0.3s ease;
                    ">
                        ${villainIsWinner ? '<h1 style="font-size: 32px; color: #ffffff; margin: 0 0 15px 0; text-shadow: 0 0 20px rgba(124, 58, 237, 0.8);">🏆 WINNER 🏆</h1>' : ''}
                        <h2 style="font-size: 24px; color: #a855f7; margin: 0 0 15px 0; text-shadow: 0 0 15px #a855f7;">👹 Villain</h2>
                        <div style="font-size: 48px; color: #ffffff; font-weight: bold; margin: 15px 0; text-shadow: 0 0 20px rgba(255, 255, 255, 0.5);">${opponentSTX}</div>
                        <div style="font-size: 16px; color: #d1d5db; margin-top: 10px;">STX Tokens Collected</div>
                    </div>
                </div>
        ...
        if (this.userMazeConfig?.winnerPosition) {
            ...
            if (submissionStatus) {
                ...
                submissionStatus.innerHTML = `
                    <div style="color: #10b981; font-size: 16px; margin-top: 20px;">
    ...
    gameWon() {
        // Compare playerSTXCount and opponentSTXCount
        const playerSTX = this.gameState.playerSTXCount || 0;
        const opponentSTX = this.opponentSTXCount || 0;
        let winner;
        if (playerSTX > opponentSTX) {
            winner = 'Player';
        } else if (opponentSTX > playerSTX) {
            winner = 'Villain';
        } else {
            winner = 'Draw';
        }
        // Show game over screen with winner info
        this.showGameOverScreen(winner, playerSTX, opponentSTX);
        this.submitScore();
    }
    ...
    showVictoryScreen() {
        ...
        if (!victoryOverlay) {
            ...
            victoryOverlay.innerHTML = `
                ...
                <div style="font-size: 48px; color: #8B5CF6; margin-top: 30px; margin-bottom: 20px; text-shadow: 0 0 25px #000000; font-weight: bold;">
    ...
    debugShowVictory(scenario = 'player') {
        console.log('🎨 DEBUG: Showing victory screen for styling');
        
        // Set up mock data based on scenario
        let playerSTX, opponentSTX;
        switch(scenario) {
            case 'player':
                playerSTX = 8;
                opponentSTX = 2;
        ...
        this.gameState.playerSTXCount = playerSTX;
        this.opponentSTXCount = opponentSTX;
    ...
    debugShowGameOver(scenario = 'player') {
        console.log('🎨 DEBUG: Showing game over screen for styling');
        ...
        this.showGameOverScreen('test', playerSTX, opponentSTX);
    }
